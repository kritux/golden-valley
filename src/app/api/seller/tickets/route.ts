...
export async function GET() {
  ...
  const { data, error } = await adminClient
    .from('tickets')
    .select(`
      id, ticket_number, status, activated_at, created_at,
      buyer:profiles!tickets_buyer_id_fkey(first_name, last_name, email, phone),
      payment:payments(amount, method, status)
    `)
